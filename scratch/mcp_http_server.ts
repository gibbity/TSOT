...
// Initialize environments and clients
...
if (!supabase) {
  console.error('⚠️ Supabase credentials missing. Running in offline/fallback mode.');
}
if (!ai) {
  console.error('⚠️ GEMINI_API_KEY missing. Generating embeddings and audit synthesis will be disabled or fall back to heuristics.');
}
  ...
  if (supabase) {
    ...
    try {
      if (queryEmbedding) {
        ...
        if (error) console.error('RPC hybrid search registry failed:', error);
      }
      ...
      if (error) console.error('Keyword search registry failed:', error);
    } catch (err) {
  ...
  if (supabase) {
    ...
    try {
      if (queryEmbedding) {
        ...
        if (error) console.error('RPC hybrid search AI Act failed:', error);
      }
      ...
      const { data, error } = await dbQuery;
      if (!error && data) return data;
      if (error) console.error('Keyword search AI Act failed:', error);
    } catch (err) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === 'audit_eu_compliance') {
      ...
      const result = await auditEuCompliance(prompt);
      return { content: [{ type: 'text', text: result }] };
    }

    if (name === 'optimize_hci_design') {
      const prompt = String(args?.prompt || '');
      const result = await optimizeHciDesign(prompt);
      return { content: [{ type: 'text', text: result }] };
    }

    if (name === 'query_research_moat') {
      const query = String(args?.query || '');
      const result = await queryResearchMoat(query);
      return { content: [{ type: 'text', text: result }] };
    }
    ...
    if (name === 'get_record') {
      ...
      const source = (args?.source || 'both') as 'corpus' | 'ai_act' | 'both';
      const record = await getRecord(code, source);
      if (!record) {
        return {
          content: [
            {
              type: 'text',
              text: `No record found with code "${code}" in source "${source}"`
            }
          ],
          isError: true
        };
...
// Register Resource Reader Handler
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  try {
    ...
    if (uri.startsWith('tsot://ai_act/article/')) {
      ...
      if (!record) {
        throw new Error(`AI Act article with code "${code}" not found`);
      }
      return {
        contents: [
          {
...
app.get('/sse', async (req, res) => {
  console.error(`🔌 New SSE connection requested. Establishing event stream...`);
  
  // Set up the SSE Server Transport
  // The first argument is the POST endpoint path that the client will hit to send messages back to the server
  const transport = new SSEServerTransport('/messages', res);
  
  // Track transport by its unique session ID
  transports[transport.sessionId] = transport;
  console.error(`✨ Session established: ${transport.sessionId}`);
  ...
  try {
    ...
    console.error(`🚀 Connected server to transport for session: ${transport.sessionId}`);
  } catch (err) {
    console.error(`❌ Failed to connect server to transport:`, err);
  }
});

// Parse raw text or JSON depending on Content-Type header
app.post('/messages', express.json(), async (req, res) => {
  ...
  if (transport) {
    ...
    console.error(`⚠️ Session ID "${sessionId}" not found. Rejected POST message.`);
...
app.listen(PORT, () => {
  console.error(`\n======================================================`);
  console.error(`🌐 TSOT Hosted SSE MCP Server running at http://localhost:${PORT}`);
  console.error(`🔗 Establish SSE transport connection at: http://localhost:${PORT}/sse`);
  console.error(`💬 POST messaging pathway at: http://localhost:${PORT}/messages`);
  console.error(`======================================================\n`);
});
