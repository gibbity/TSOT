...
async function exportComplianceDossier(systemName: string, systemDescription: string, intendedPurpose?: string) {
  let relevantArticles = SEED_AI_ACT_RECORDS.slice(0, 8);
  if (supabase) {
    try {
      const { data } = await supabase.from('ai_act').select('*').limit(8);
      if (data && data.length > 0) relevantArticles = data;
    } catch {}
  }
...
- **Category / Tier:** \`${art.pillar || (art as any).category || 'LIMITED RISK'}\` (Risk Level: \`${art.risk_level}\`)
- **Statutory Mandate:** ${art.human_summary?.slice(0, 300)}...
- **Engineering Verdict:** ${art.verdict}
`).join('\n')}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (method === 'tools/call') {
      ...
      } else if (name === 'export_compliance_dossier') {
        const systemName = String(args?.system_name || 'AI System');
        const systemDescription = String(args?.system_description || '');
        const intendedPurpose = String(args?.intended_purpose || '');
        textOutput = await exportComplianceDossier(systemName, systemDescription, intendedPurpose);
      } else {
